///هذا البوت الاصدار الثاني من بوت السيستم تحت ادارة حكيم حسام :جميع الحقوق محفوظة وموجودة
///اخر تحديث 8/6\2020
...
const youtube = new YouTube("AIzaSyBIYX33o-UNP_P_3H4Ag0NWTkMq7w9mki8"); //تعديل اساسي اي بي اي يوتيوب
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyBIYX33o-UNP_P_3H4Ag0NWTkMq7w9mki8"; ///تعديل اساسي اي بي اي يوتيوب
...
client.on("ready", () => {
  ...
  client.user.setActivity(`Palestine S`, { type: "PLAYING" });
});
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "new")) {
    const reason = message.content
      ...
      .slice(1)
      .join(" ");
    if (!message.guild.roles.exists(gg => gg.name === "◈ 𝐒𝐮𝐩𝐩𝐨𝐫𝐭 𝐓𝐞𝐚𝐦"))
      return message.channel.send(`لازم تسوي رتبة اسمها \`◈ 𝐒𝐮𝐩𝐩𝐨𝐫𝐭 𝐓𝐞𝐚𝐦\`.`);
    if (
    ...
    message.guild
      ...
      .then(c => {
        let role = message.guild.roles.find(gg => gg.name === "◈ 𝐒𝐮𝐩𝐩𝐨𝐫𝐭 𝐓𝐞𝐚𝐦");
    ...
    if (
      !message.member.roles.has(
        message.guild.roles.filter(r => r.name === "◈ 𝐒𝐮𝐩𝐩𝐨𝐫𝐭 𝐓𝐞𝐚𝐦").first().id
      )
    )
      return message.channel.send("You don't have the `◈ 𝐒𝐮𝐩𝐩𝐨𝐫𝐭 𝐓𝐞𝐚𝐦` role!");
    message.channel
      .delete()
  ...
  if (pixelbot.content.startsWith(prefix + "user")) {
    ...
    pixelbot.guild.fetchInvites().then(invites => {
      ...
      var roles = pixelbot.member.roles
        ...
        .join(` `);
      let pixeluser = new Discord.RichEmbed() // hakeem - Codes Team.
        .setColor("#00000")
        ...
        .addField("**✽ ID :** ", pixelbot.author.id, true) // hakeem - Codes Team.
        .addField(
          "**✽ Joined At :**   ",
        ...
        .setTimestamp(); // hakeem - Codes Team.

      pixelbot.channel.sendEmbed(pixeluser).then(c => {}); // hakeem - Codes Team.
    });
  }
}); // hakeem - Codes Team.
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@540870950225313798>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  ...
  if (command === prefix + "unmute") {
    ...
    let muteRole = client.guilds
      ...
      .roles.find(gg => gg.name === "◈ 𝐌𝐮𝐭𝐞𝐝 ⚓︎");
    if (!muteRole)
      return message
        .reply("** لا يوجد لديك رتبه الميوت '◈ 𝐌𝐮𝐭𝐞𝐝 ⚓︎' **")
        .catch(console.error);
...
client.on("message", message => {
  ...
  if (command === prefix + "mute") {
    ...
    if (!message.member.hasPermission("MANAGE_ROLES"))
      return message
        .reply("** لا يوجد لديك صلاحيات 'Manage Roles' **")
        .catch(console.error);
    let user = message.mentions.users.first();
    let modlog = client.channels.find(gg => gg.name === "log");
    let muteRole = client.guilds
    ...
    if (!muteRole)
      return message
        .reply("** لا يوجد رتبة الميوت '◈ 𝐌𝐮𝐭𝐞𝐝 ⚓︎' **")
        .catch(console.error);
    if (message.mentions.users.size < 1)
      return message
        .reply("** يجب عليك منشنت شخص اولاً**")
    ...
    if (
      ...
      return message
        .reply("** لا يوجد لدي صلاحيات لاسكاته :) Manage Roles **")
        .catch(console.error);
        ...
        if (message.mentions.members.first().voiceChannel != null) {
          ...
          message.channel.send(
            "`You Cant Move" +
              ...
              " `يجب أن يكون المستخدم في اي روم صوتي لنقله`"
          );
        }
      } else {
        message.channel.send(
          "**``يجب أن تكون في صوت الغرفة لنقل اعضاء اليك``**"
        );
      }
    } else {
      message.react("❌");
...
///تعديل غير اساسي
//اذا صاؤ مشكلة انو ما بيرسل اوامر ادارية بس اكتب d عند كلمو sen
////كود هيلب
client.on("message", message => {
  if (message.author.bot) return;
...
\`${prefix}avt\` :عشان يعرض لك صورت  اي شخص عن طريق الايدي 
...
\`${prefix}setVc\` معطل <channel name> : لتحديد روم الفويس اونلاين 
\`${prefix}vc off\` :(معطل) لإغلاق روم الفويس اونلاين
...
\`شرح البوت \` :  اتواصل مع المطور تبع البوت او فتح تكت هون https://discord.gg/bjGGyCg 
`);
        })
        .then(e => {
          message.react("✅");
...
\`${prefix}np\` : عرض مايتم تشغيله الان [np] 
\`${prefix}repeat\` : تكرار الاغنية 
\`شرح البوت \` :فتح تكت في هاد السيرفر ل اي مساعدة https://discord.gg/bjGGyCg
...
client.on("message", message => {
  if (message.content === "حكيم") {
    message.channel.send("**:heart:تفضل ؟:heart:**");
    message.channel.sendFile("");
  }
});
...
client.on("message", message => {
  if (message.content === "hakeem") {
    message.channel.send("**:heart:Yes ?:heart:**");
    message.channel.sendFile("");
  }
});
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setby")) {
    ...
    client.on("guildMemberAdd", member => {
      ...
      var channel = member.guild.channels.find(gg => gg.name === "𓇽-𝗟𝗼𝗴"); //// تعديل اساسي
      if (!channel) return;
...
client.on("guildMemberAdd", async member => {
  ...
  if (channel) {
    ...
    //Avatar
    var imageX = 174; //X coordinate
    var imageY = 165; //Y coordinate
    var imageRadius = 32; //نصف قطر الصورة الدائرية

    //Member Name
    var nameSize = "29pt"; //حجم خط الاسم
    var nameKind = "Source Sans Pro (OT1)"; //نوع خط الاسم
    var nameColor = "#04F1B4"; //لون خط الاسم

    //Name Position
    var nameX = 44; //position x
    var nameY = 186; //position y
